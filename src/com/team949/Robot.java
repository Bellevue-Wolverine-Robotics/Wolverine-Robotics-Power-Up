...
public class Robot extends TimedRobot {
	...
	public void robotInit() {
		...
		UsbCamera driveCamera = CameraServer.getInstance()
				.startAutomaticCapture(1);

		driveCamera.setFPS(15);
		driveCamera.setResolution(160, 120);

		UsbCamera armCamera = CameraServer.getInstance()
		...
		SmartDashboard.putData("Auto: Starting Position",
				this.startingPositionChooser);
		...
		SmartDashboard.putData("Auto: Target Scoring",
				this.targetScoringChooser);
	...
	public void autonomousInit() {
		...
		autonomousCommand = autonomousChooseCommand(startingPosition,
	...
	private Command autonomousChooseCommand(char startingPosition,
			String targetScoring, String gameData) {
		...
		if (startingPosition == L) // Go from left
		{
			...
			if (targetScoring.equals(SCALE)) {
				if (robotOnTheSameSide(startingPosition, scalePosition)) {
					DriverStation.reportWarning("EXECUTING: LeftSideLeftScale",
							false);
					return new LeftSideLeftScale();
				} else {
					DriverStation.reportWarning("EXECUTING: LeftSideRightScale",
							false);
					// return new AutoLine();
		...
		} else if (startingPosition == M) // Go from middle
		{
			...
			// if(switchPosition == L)
			// {
			// DriverStation.reportWarning("EXECUTING: MiddleLeftSwitch",
			// false);
			// return new MiddleLeftSwitch();
			// }
			// else
			// {
			// DriverStation.reportWarning("EXECUTING: MiddleRightSwitch",
			// false);
			...
			if (targetScoring.equals(SCALE)) {
				if (robotOnTheSameSide(startingPosition, scalePosition)) {
					DriverStation.reportWarning(
							"EXECUTING: RightSideRightScale", false);
					return new RightSideRightScale();
				} else {
					DriverStation.reportWarning("EXECUTING: RightSideLeftScale",
							false);
					// return new AutoLine();
	...
	private boolean robotOnTheSameSide(char robotPosition,
			char targetPosition) {
	...
	private Command armL = new ArmLower(), armR = new ArmRaise(),
			wristStow = new HandStow(), shoot = new DelayedShoot(0.5, -0.5, 1);
	CommandGroup switchLaunch = new SwitchLaunch();
